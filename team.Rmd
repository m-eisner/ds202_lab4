...
dat %>% ggplot(aes(x=Iowa, y=`Notre Dame`)) + geom_point() #+ geom_smooth(method = "lm", se=FALSE)
```
...
fantasy %>% 
  ...
  group_by(Name) %>%
  summarize(mean.points.per.game.2018=mean(fantasy_points_2018), mean.points.per.game.2019=mean(fantasy_points),
            total.points.2018=sum(fantasy_points_2018), total.points.2019=sum(fantasy_points))
```
